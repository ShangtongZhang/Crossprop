...
def collect(stride, units, stepSize, nSample):
    ...
    for startRun in range(0, runs, stride):
        path = 'partial/YAD_partial_'+str(startRun)+'_'+str(units)+'_'+str(stepSize)+'_'+str(nSample)+'.bin'
    ...
    fw = open('partial/YAD_total_'+str(units)+'_'+str(stepSize)+'_'+str(nSample)+'.bin', 'wb')
