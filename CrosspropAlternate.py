...
class Crossprop_Alternate():
    def __init__(self, input_dim=20, hidden_dim=1000, output_dim=10, alpha_step_size=0.001, beta_step_size=0.001,
                 lmbda=0.0, non_linearity='tanh'):
        ...
        self.hidden_output_weights = np.random.normal(0.0, 1.0, size=(self.hidden_dim, self.output_dim))
        self.input_hidden_weights = np.random.normal(0.0, 1.0, size=(self.input_dim, self.hidden_dim - 1))
        # self.hidden_output_weights = np.ones((self.hidden_dim, self.output_dim))
        # self.input_hidden_weights = np.ones((self.input_dim, self.hidden_dim - 1))
    ...
    def crosspropagate_errors(self, true_label):
        ...
        for i in range(self.output_dim):
            ...
            input_hidden_weights_interim[0: self.hidden_dim - 1, 0] += (
            delta[i] * (self.features[0: self.hidden_dim - 1].flatten() * self.input_hidden_traces[:, i]))
            self.input_hidden_traces[:, i] = self.input_hidden_traces[:, i] * (
            1.0 - self.beta_step_size * (self.features[0: self.hidden_dim - 1].flatten() ** 2)) - \
                                             (self.beta_step_size * delta[i])
        input_hidden_weights_delta = np.dot(self.input_vector,
                                            (input_hidden_weights_interim * self.get_nonlinearity_derivative()[
                                                                            0: self.hidden_dim - 1, :]).transpose())
